...
public final class EncodingSniffer {
    ...
    static String extractEncodingFromContentType(final String s) {
        ...
        final byte[] bytes = Utils.getBytes(s);
        int i;
        ...
        if (bytes[i] == '"') {
            ...
            final String charset = Utils.getUTF8(subarray(bytes, i + 1, index));
        ...
        final String charset = Utils.getUTF8(subarray(bytes, i, end));
        return isSupportedCharset(charset) ? charset : null;
    }
    ...
    static String sniffEncodingFromXmlDeclaration(final byte[] bytes) {
        String encoding = null;
        final byte[] declarationPrefix = Utils.getBytes("<?xml ");
        if (isEquals(declarationPrefix, subarray(bytes, 0, declarationPrefix.length))) {
            final int index = indexOf(bytes, (byte) '?', 2);
            if (index + 1 < bytes.length && bytes[index + 1] == '>') {
                final String declaration = Utils.getUTF8(bytes, 0, index + 2);
    ...
    public static boolean isEquals(final byte[] array1, final byte[] array2) {
